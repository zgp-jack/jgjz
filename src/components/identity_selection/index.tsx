...
import { IdentityProps } from './index.d'
import './index.scss'
    ...
    <Block>
      {show &&
        <View className='identity-selection-box'>
          <Image className="identity-back-icon" onClick={() => setShow && setShow(false)} src={`${IMGCDNURL}common/left-arrow.png`}></Image>
          <View className="identity-book-item">
            ...
            <View className="identity-flex">
              {createConfig.map(item => (
                <View className="identity-selection-type" onClick={() => createAccountBook(item)}>
