    ...
    <View className='prompt-container' style={{ paddingBottom: focusStyle ? "400rpx":"0"}}>
      <View className='prompt-box' >
        ...
        <View className='prompt-content'>     
          {inputGroup.map((item)=>(
            item.title ? (<View className='input-container' key={item.name}><View className='input-title' >{item.title}</View><Input type='text' placeholder={item.placeholder} data-name={item.name} maxLength={item.maxlength || inputMaxLength} value={item.value} onInput={(e) => enterInput(e)} onFocus={inputFocus} onBlur={inputBlur} focus={item.name == 'name' ? true : false} disabled={ !!item.disabled}   ></Input></View>) 
            : 
              <Input key={item.name} type='text' placeholder={item.placeholder} maxLength={item.maxlength || inputMaxLength} disabled={!!item.disabled} data-name={item.name} value={item.value} onInput={(e) => enterInput(e)} onFocus={inputFocus} onBlur={inputBlur}></Input>
          ))}
        </View>
